...
// Owner numbers
const ownerNumber = ["923404923915"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("TFMDBOT~")) {
    console.log("❌ Invalid TFMDBOT session format");
    process.exit(1);
  }
  ...
  console.log("♻️ TFMDBOT session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'Tahir Iqbal'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 TFMDBOT Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ TFMDBOT Bot connected to WhatsApp!");
